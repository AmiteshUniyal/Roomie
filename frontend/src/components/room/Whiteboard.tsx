...
export default function Whiteboard({ roomId, isConnected }: WhiteboardProps) {
  ...
  const otherUsersDrawingRef = useRef<Map<string, { x: number; y: number }>>(
    new Map()
  );
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      ...
      // Re-apply current settings after resize
  ...
  // Update canvas context when color/lineWidth changes
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleDrawing = (data: DrawingData) => {
      ...
      if (drawData.type === "start") {
        ...
        otherUsersDrawingRef.current.set(data.userId, {
          x: drawData.x,
          y: drawData.y,
        });
    ...
    const handleCanvasStateLoaded = (
      data: SocketEvents["canvas:state:loaded"]
    ) => {
      ...
      data.strokes.forEach((stroke) => {
        ...
        if (stroke.type === "start") {
          ...
          otherUsersDrawingRef.current.set(stroke.userId, {
            x: stroke.x,
            y: stroke.y,
          });
  ...
  const startDrawing = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    console.log(
      `🎨 Starting drawing at (${x}, ${y}) with tool: ${selectedTool}`
    );

...
export default function RoomSidebar({ room, documents, activeDocument, onDocumentChange, isConnected, roomMembers = [] }: RoomSidebarProps) {
    ...
    const [activeTab, setActiveTab] = useState<'documents' | 'members' | 'requests'>('documents');
    ...
    const handleDeleteDocument = async (documentId: string) => {
        ...
        try {
            ...
            if (activeDocument?.id === documentId) {
                const newActiveDocument = updatedDocuments.length > 0 ? updatedDocuments[0] : null;
                ...
                <div className="flex space-x-1">
                    ...
                    {!room.isPublic && (
                        <button
                            ...
                            className={`flex-1 px-3 py-2 text-sm font-medium rounded-md transition-colors ${activeTab === 'requests'
                                ? 'bg-blue-100 text-blue-600'
                                : 'text-gray-600 hover:text-gray-900'
                                }`}
                        >
                ...
                {activeTab === 'documents' ? (
                    <div className="p-4">
                        ...
                        <div className="space-y-2">
                            {documents.map((document) => (
                                <div
                                    key={document.id}
                                    className={`p-3 rounded-lg cursor-pointer transition-colors border ${activeDocument?.id === document.id
                                ...
                                >
                                    <div className="flex items-center justify-between">
                                        ...
                                        >
                                            {isDeletingDocument === document.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Trash className="w-4 h-4" />}
                                        </button>
                                    </div>
                                </div>
                            ))}
                        ...
                        {documents.length === 0 && (
                            <div className="text-center py-8">
                                ...
                                <p className="text-gray-400 text-xs">Create your first document to get started</p>
                            </div>
                        )}
                    </div>
                ) : activeTab === 'members' ? (
                    <div className="p-4">
                        ...
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <span className="text-sm text-gray-500">{roomMembers.length} active</span>
                        </div>
